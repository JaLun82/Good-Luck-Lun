...
let finChartBreakdown = null; // 「各目標缺口組成」長條圖的 Chart.js 實例
let finChartTrend = null; // 「資金累積曲線」折線圖的 Chart.js 實例，方案A/B切換時整個重建
...
const FIN_CHART_COLORS = ['#4a7fa5', '#27ae60', '#e8a33d', '#c0392b', '#8e44ad', '#16a085', '#d35400', '#2c3e50'];

/* 滑鼠移到圖表上時，畫一條垂直虛線標出目前指到的位置，
   方便同時對照多條線（方案B多目標）或同一根長條的疊加數值。 */
const finCrosshairPlugin = {
  id: 'finCrosshair',
  afterDraw(chart) {
...
function renderFinCharts(results) {
  if (typeof Chart === 'undefined') return; // CDN 尚未載入完成或載入失敗時直接跳過，不影響其他區塊
  renderBreakdownChart(results);
  renderTrendChart();
}

/* 長條圖：每個目標的 want 拆成「已分配既有資產」＋「缺口（need）」兩段疊加，
   一眼看出每個目標還缺多少、既有資產又幫忙補了多少。方案B下 allocate 固定是0，
   長條會整條都是缺口，剛好呼應「分開達成不扣既有資產」的邏輯。 */
function renderBreakdownChart(results) {
  const canvas = $F('fin-chart-breakdown');
  if (!canvas) return;

  if (!finGoals.length) {
    if (finChartBreakdown) { finChartBreakdown.destroy(); finChartBreakdown = null; }
    return;
  }

  const sorted = [...finGoals].sort((a, b) => a.targetAge - b.targetAge);
  const labels = sorted.map(g => `${g.name}（${g.targetAge}歲）`);
  const allocateData = sorted.map(g => Math.round(results.get(g.id)?.allocate || 0));
  const needData = sorted.map(g => Math.round(Math.max(results.get(g.id)?.need || 0, 0)));

  if (finChartBreakdown) finChartBreakdown.destroy();
  finChartBreakdown = new Chart(canvas.getContext('2d'), {
    type: 'bar',
    data: {
      labels,
      datasets: [
        { label: '已分配既有資產', data: allocateData, backgroundColor: '#4a7fa5', borderRadius: 4 },
        { label: '缺口（need）', data: needData, backgroundColor: '#e8a33d', borderRadius: 4 },
      ],
    },
    plugins: [finCrosshairPlugin],
    options: {
      responsive: true,
      ...
      scales: {
        x: { stacked: true },
        y: { stacked: true, ticks: { callback: v => Number(v).toLocaleString('zh-TW') } },
      },
      plugins: {
...
/* 折線圖：資金隨年齡累積的軌跡，依目前選擇的方案切換內容——
   方案A：所有目標共用一筆資金池，畫一條線（用逐年試算表同一份資料，跟下載的Excel完全對得上）；
   方案B：每個目標各自一條線，各自從現在累積到自己的達成年齡（同樣用 buildPerGoalProjection，
   跟Excel每個目標的工作表資料一致）。 */
function renderTrendChart() {
  const canvas = $F('fin-chart-trend');
  const titleEl = $F('fin-chart-trend-title');
  const descEl = $F('fin-chart-trend-desc');
  if (!canvas) return;

  if (finChartTrend) { finChartTrend.destroy(); finChartTrend = null; }

  if (!finGoals.length) return;

  if (finSelectedPlan === 'B') {
    if (titleEl) titleEl.textContent = '各目標資金累積曲線（方案B：分開達成）';
    if (descEl) descEl.textContent = '每個目標各自從現在開始累積，終點為該目標的達成年齡';

    const currentAge = getNumF('fin-current-age');
    const { results } = calcAllGoals();
    const datasets = finGoals.map((goal, i) => {
      const r = results.get(goal.id);
      const rows = buildPerGoalProjection(goal, r, currentAge);
      const color = FIN_CHART_COLORS[i % FIN_CHART_COLORS.length];
      return {
        label: goal.name,
        data: rows.map(row => ({ x: row.年齡, y: row.年末累積餘額 })),
        borderColor: color,
    ...
    finChartTrend = new Chart(canvas.getContext('2d'), {
      type: 'line',
      data: { datasets },
      plugins: [finCrosshairPlugin],
      options: finTrendChartOptions(),
    });
  } else {
    if (titleEl) titleEl.textContent = '資金池累積曲線（方案A：一起達成）';
    if (descEl) descEl.textContent = '所有目標共用同一筆資金池，餘額於各目標到期年齡提領一筆';
    ...
    finChartTrend = new Chart(canvas.getContext('2d'), {
      ...
      data: {
        datasets: [{
          label: '資金池餘額',
          data: rows.map(row => ({ x: row.年齡, y: row.年末總餘額 })),
          borderColor: '#4a7fa5',
...
function finTrendChartOptions() {
  return {
    responsive: true,
    maintainAspectRatio: false,
    interaction: { mode: 'index', intersect: false },
    scales: {
      x: { type: 'linear', title: { display: true, text: '年齡', font: { size: 11 } }, ticks: { stepSize: 1 } },
      y: { title: { display: true, text: '餘額（元）', font: { size: 11 } }, ticks: { callback: v => Number(v).toLocaleString('zh-TW') } },
    },
    plugins: {
      legend: { position: 'bottom', labels: { font: { size: 11 } } },
      tooltip: { callbacks: { label: ctx => `${ctx.dataset.label}：${Math.round(ctx.raw.y).toLocaleString('zh-TW')} 元（${ctx.raw.x}歲）` } },
    },
  };
}
